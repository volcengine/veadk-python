...
test("Studio access fails closed until the server-derived role is known", () => {
  assert.match(clientSource, /export type StudioRole = "super_admin" \| "admin" \| "developer" \| "user"/);
...
test("only administrators and developers receive Agent deployment controls", () => {
  ...
  assert.match(myAgentsSource, /if \(activeType === "general" && canCreateRuntimeAgents\)[\s\S]*?createAgent = \(\) => onCreateAgent\(region, "general"\)/);
  assert.match(myAgentsSource, /activeType === "mpa"[\s\S]*?canCreateRuntimeAgents[\s\S]*?setMpaCreateRegion\(region\)/);
