...
test("finds the first missing transient model credential with its agent path", () => {
  const draft = {
    name: "Root",
  ...
  assert.deepEqual(
    ...
    {
      key: "CUSTOM_MODEL_WORKER_API_KEY",
      label: "Worker model API Key",
      path: [0],
    },
  );
  assert.equal(
