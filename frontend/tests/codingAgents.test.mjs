...
test("registers Coding Agents as a local configuration automation", () => {
  ...
  assert.match(definitionSource, /name: "Configure coding agents"/);
...
test("only enables Coding Agents configuration on 127.0.0.1", async () => {
  ...
  assert.match(applicationsSource, /t\("localOnly"\)/);
...
test("renders client and bundled skill selection with one global configure action", () => {
  ...
  assert.match(integrationSource, /aria-label=\{t\("codingAgents\.clients\.ariaLabel"\)\}/);
  assert.match(integrationSource, /aria-label=\{t\("codingAgents\.skills\.ariaLabel"\)\}/);
  assert.match(integrationSource, /aria-pressed=\{selectedAgentIds\.has\(agent\.id\)\}/);
  assert.match(integrationSource, /type="checkbox"/);
  assert.match(integrationSource, /t\("codingAgents\.skills\.viewFiles"\)/);
  assert.match(integrationSource, /SkillPreviewDialog/);
  assert.match(integrationSource, /t\("codingAgents\.global\.title"\)/);
  assert.match(integrationSource, /globalSkillsPath/);
  assert.match(integrationSource, /t\("codingAgents\.configuring"\)/);
  assert.match(integrationSource, /actionBusy \? t\("codingAgents\.configuring"\) : t\("codingAgents\.configure"\)/);
