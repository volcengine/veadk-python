...
- **新会话模式**：保留 Agent 对话模式；临时会话模式显示为接入中；Skill 创建模式会在两个独立的 AgentKit CodeEnv Session 中使用固定模型做真实 A/B 生成，完成后可对比、下载 ZIP 或添加到 AgentKit。
- **Agent 选择器**：左上角切换 agent；长列表在视口内独立滚动，悬停可查看该 agent 的模型与挂载的工具。
- **历史会话**：自动保存、按时间排序，可重新打开或删除。
- **灵光一现**：从新会话输入框或全局标题栏启动独立的 AgentKit 沙箱临时会话。
  Studio 会复用专属 CodeEnv Tool、为当前用户创建全新的 Sandbox Session，并通过
  Codex 进行多轮对话。退出后云端 Session 会被删除，内容不会写入普通历史会话；
...
启动前必须在服务端配置一个专用且状态为 `Ready` 的 CodeEnv Tool：
...
本地运行前使用 AgentKit 的 `credential-hosting` 命令，并选择将结果写入上述 CodeEnv
Tool。
也可使用兼容变量 `AGENTKIT_SANDBOX_TOOL_ID`。添加到 AgentKit 时，服务端会先将
ZIP 上传到 TOS，再调用 Skills API 创建或更新 Skill；只有用户填写 SkillSpace ID
时才发布到对应空间。以下变量均为可选覆盖项：

| 环境变量 | 默认值 | 说明 |
| :-- | :-- | :-- |
| `VEADK_SKILL_CREATOR_TOOL_ID` | — | 专用 AgentKit CodeEnv Tool ID。 |
...
云上部署时使用 `--skill-creator-tool-id`。部署命令会用部署凭据获取 Ark API Key，
通过 AgentKit 凭据托管将其保存到 KMS 并准备网关代理，再把代理地址和可撤销票据
绑定到专用 CodeEnv Tool；VeFaaS Function 不接收模型凭据：
