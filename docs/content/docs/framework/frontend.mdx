...
`--admin` 和 `--developer` 分别接收逗号分隔的用户名或 OAuth 邮箱。空格会被
忽略，身份匹配不区分大小写；同一身份同时出现在两个名单时，`admin` 优先。
本地启动示例：
...
也可以分别通过 `VEADK_STUDIO_ADMINS` 和 `VEADK_STUDIO_DEVELOPERS` 环境变量
设置这两个名单。
...
`veadk:author` 归属标签的历史 Runtime 仅 admin 可见。
