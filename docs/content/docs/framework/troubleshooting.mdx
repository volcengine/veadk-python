---
title: "常见问题"
description: "VeADK 在环境准备、开发与部署环节的常见问题与解决方案。"
---

本页汇总你在使用 VeADK 时可能遇到的常见问题。按环境准备、开发、部署分组。

<Accordions>
  <Accordion title="VeADK 与 Google ADK 的关系是什么，是否兼容？">
...
<Accordions>
  <Accordion title="使用知识库时，提示未找到 llama-index 等包">
    请安装 VeADK 扩展包：
...
<Accordions>
  <Accordion title="在 Release 阶段卡住，云上没有报错">
    请先在 VeFaaS 平台开通权限。

    ![开通权限](/assets/images/faq/troubleshooting-01.png)
  </Accordion>
  <Accordion title="新账号开通后，缺少 ServerlessApplicationRole 授权">
  ...
  <Accordion title="安装依赖失败，显示依赖安装空间不足">
    VeFaaS 最大依赖安装大小默认为 1024 MB，若需更大空间，请联系火山引擎 VeFaaS 产品团队扩容。
  </Accordion>
  <Accordion title="错误日志显示 [apig_gateway][][get_info][Error] ExceededQuota">
    ![网关超额报错](/assets/images/faq/troubleshooting-03.jpeg)

    火山引擎 APIG 中，Serverless 类型的网关实例最多只能有 1 个。您需要在部署时指定唯一的网关实例。
  </Accordion>
</Accordions>
