---
title: "A2A Agent"
description: "基于 Agent-to-Agent (A2A) 协议将 VeADK Agent 开放为服务并跨网络互联，含鉴权与自定义 HTTP 客户端。"
---
...
| **微服务架构** | 不同的 Agent 由不同的团队或组织维护，A2A 用于这些服务跨网络边界相互通信 |
...
<Accordions>
<Accordion title="火山引擎 VeFaaS 采用的默认鉴权方式">
...
在 VeADK 中，主要通过 `RemoteVeAgent` 类来实现自定义 HTTP 客户端的配置，它提供了一个 `httpx_client` 参数，允许您传入预配置的 `httpx.AsyncClient` 实例。这使得您可以灵活地控制 HTTP 请求的各种参数，如代理设置、超时控制、连接池管理等，从而更好地适应不同的网络环境和需求。

您可以参考以下示例代码创建自定义的 HTTP 客户端，可配置各种 HTTP 客户端参数，如：
...
custom_client = httpx.AsyncClient(
    ...
    verify=True,
    # 代理配置（如需使用代理；httpx >= 0.28 使用 proxy 参数）
