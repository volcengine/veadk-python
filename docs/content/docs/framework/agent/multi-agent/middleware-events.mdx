...
调用 `listen()` 后，客户端会在指定 topic 上启动消费者，每当有新消息到达，便回调 `recv_msg_callback` 来驱动 Agent 处理，从而实现 Agent 之间的异步、事件驱动协作。
