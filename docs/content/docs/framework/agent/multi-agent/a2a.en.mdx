...
sequenceDiagram
    ...
    participant WeatherTool as get_city_weather tool

    User->>Client: Run query ("What is the weather like of Beijing?")
    Client->>RootAgent: Create and configure the Agent
    Client->>RemoteAgent: Create remote Agent connection (http://localhost:8000/)
    RootAgent->>RemoteAgent: Add as sub-agent (sub_agents)
    Client->>RootAgent: Run the query via Runner
    RootAgent->>RemoteAgent: Call to fetch weather
    RemoteAgent->>Server: HTTP request (A2A protocol)
    Server->>WeatherAgent: Forward the request
    WeatherAgent->>WeatherTool: Call the get_city_weather tool
    WeatherTool-->>WeatherAgent: Return weather data
    WeatherAgent-->>Server: Processed result
    Server-->>RemoteAgent: HTTP response
    RemoteAgent-->>RootAgent: Return result
    RootAgent-->>Client: Return final response
    Client-->>User: Show weather info
```
...
custom_client = httpx.AsyncClient(
    ...
    verify=True,
    # Proxy configuration (if a proxy is needed; httpx >= 0.28 uses the `proxy` parameter)
