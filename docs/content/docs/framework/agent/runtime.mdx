...
它们把整个内层循环交给外部 agent harness，因此 ADK 在该流程内实现的大部分能力——请求/响应处理器（planner、code_executor、example store、知识库注入）、逐次模型调用的回调与 span——都不会执行。基础 Agent transfer 会通过 `transfer_to_agent` 桥接回 ADK，但这些运行时仍更适合「模型可以在受控沙箱里跑命令、改文件」的场景，而不是需要完整 ADK flow 的场景。具体差异见下方[支持矩阵](#支持矩阵)。
</Callout>
...
- **需要 `output_schema`、`planner`、`code_executor` 或高级 `generate_content_config` 的智能体。** 这些配置在非 adk 运行时下会**直接报错**，而不是降级——完整清单见[支持矩阵](#支持矩阵)。
...
<Callout type="info">
`sub_agents` 会通过 ADK 兼容的 `transfer_to_agent` 桥接支持。外部运行时可以请求转交，VeADK 再在外层 ADK 上下文中运行被选中的目标智能体。
</Callout>
