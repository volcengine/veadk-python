...
它们把整个内层循环交给外部 agent harness，因此 ADK 请求/响应处理器（planner、code_executor、example store、知识库注入）、逐次模型调用的回调与 span 不会在这层流程里执行。VeADK 会把 `sub_agents` 桥接成 `transfer_to_agent` 工具，但这些运行时仍更适合「模型需要在受控沙箱里跑命令、改文件」的任务，而不是所有 ADK 能力的透明后端替换。具体差异见下方[支持矩阵](#支持矩阵)。
</Callout>
...
- **需要 `output_schema`、`planner`、`code_executor` 或 `generate_content_config` 的智能体。** 这些配置在非 adk 运行时下会**直接报错**，而不是降级——完整清单见[支持矩阵](#支持矩阵)。
