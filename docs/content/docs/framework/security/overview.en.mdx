---
title: "Overview"
description: "A map of VeADK security: inbound auth, outbound auth, permission policy, and trusted execution."
---
...
| :--- | :--- | :--- |
| **Inbound auth** | Verify the caller's identity and protect access to your agent | [Inbound auth](/en/docs/framework/security/inbound) |
...
| **Permission policy** | Decide by policy whether a user may invoke a given agent / tool | [Permission policy](/en/docs/framework/security/permission-policy) |
| **Trusted execution** | An end-to-end encrypted channel with verifiable component identity | [Trusted MCP](/en/docs/framework/security/outbound/trusted-mcp) |
...
See [Inbound auth](/en/docs/framework/security/inbound).
...
| :--- | :--- | :--- |
| **API Key** | Static credentials, service-to-service, simplest | [API Key](/en/docs/framework/security/outbound/api-key) |
| **OAuth2 M2M** | Backend-to-backend auth with automatic token refresh | [OAuth2 M2M](/en/docs/framework/security/outbound/oauth2-m2m) |
| **OAuth2 user federation** | Access third-party services on a user's behalf, with user consent | [OAuth2 user federation](/en/docs/framework/security/outbound/oauth2-user-federation) |
...
A: For fixed credentials with no refresh, use [API Key](/en/docs/framework/security/outbound/api-key); for service-to-service auth with short-lived, auto-refreshed tokens, use [OAuth2 M2M](/en/docs/framework/security/outbound/oauth2-m2m); to act on behalf of a specific user, use [OAuth2 user federation](/en/docs/framework/security/outbound/oauth2-user-federation).
...
See [Permission policy](/en/docs/framework/security/permission-policy).
...
See [Trusted MCP](/en/docs/framework/security/outbound/trusted-mcp).

## Next steps

<Cards>
  <Card title="Inbound auth" href="/en/docs/framework/security/inbound" description="Protect access to your agent with API Key or OAuth2." />
  <Card title="Outbound: API Key" href="/en/docs/framework/security/outbound/api-key" description="The simplest outbound auth method." />
