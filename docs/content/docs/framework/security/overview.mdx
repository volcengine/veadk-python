---
title: "概述"
description: "VeADK 安全的整体地图：入站认证、出站认证、权限策略与可信执行。"
---
...
| 平面 | 解决的问题 | 文档 |
| :--- | :--- | :--- |
| **入站认证** | 验证调用方身份，保护对智能体的访问 | [入站认证](/cn/docs/framework/security/inbound) |
| **出站认证** | 让智能体安全地代表自己或用户访问第三方服务 | 见下方三种凭据方式 |
| **权限策略** | 基于策略决定某用户是否可调用某智能体 / 工具 | [权限策略](/cn/docs/framework/security/permission-policy) |
| **可信执行** | 端到端加密、可验证组件身份的可信通道 | [Trusted MCP](/cn/docs/framework/security/outbound/trusted-mcp) |
...
参见[入站认证](/cn/docs/framework/security/inbound)。
...
| 方式 | 适用场景 | 文档 |
| :--- | :--- | :--- |
| **API Key** | 固定凭证、服务间通信，最简单 | [API Key](/cn/docs/framework/security/outbound/api-key) |
| **OAuth2 M2M** | 后端服务间认证，支持令牌自动刷新 | [OAuth2 M2M](/cn/docs/framework/security/outbound/oauth2-m2m) |
| **OAuth2 用户委托** | 代表用户访问第三方服务，需用户授权 | [OAuth2 用户委托](/cn/docs/framework/security/outbound/oauth2-user-federation) |
...
A：凭证固定、无需刷新选 [API Key](/cn/docs/framework/security/outbound/api-key)；需要短期令牌与自动刷新的服务间认证选 [OAuth2 M2M](/cn/docs/framework/security/outbound/oauth2-m2m)；需代表某个用户访问选 [OAuth2 用户委托](/cn/docs/framework/security/outbound/oauth2-user-federation)。
...
参见[权限策略](/cn/docs/framework/security/permission-policy)。

## 可信执行

需要端到端加密、并验证通信双方身份时，用 Trusted MCP——在标准 MCP 之上扩展组件间的身份证明与加密通道。结合机密计算可让智能体与模型运行在可信环境中。

参见 [Trusted MCP](/cn/docs/framework/security/outbound/trusted-mcp)。

## 下一步

<Cards>
  <Card title="入站认证" href="/cn/docs/framework/security/inbound" description="用 API Key 或 OAuth2 保护对智能体的访问。" />
  <Card title="出站认证：API Key" href="/cn/docs/framework/security/outbound/api-key" description="最简单的出站认证方式。" />
  <Card title="权限策略" href="/cn/docs/framework/security/permission-policy" description="基于 Cedar 的细粒度授权。" />
</Cards>
