---
title: "埋点字段说明"
description: "了解 VeADK 中各类 Span 的埋点时机与字段定义，包括通用类、LLM 类与 Tool 类字段。"
---
...
- [调用 Agent 时](https://github.com/google/adk-python/blob/v1.17.0/src/google/adk/runners.py)：在 Runner 的 `_run_with_trace` 函数中埋点，Span 名称为 `invocation`
- [运行 Agent 时](https://github.com/google/adk-python/blob/v1.17.0/src/google/adk/agents/base_agent.py)：在 Agent 的 `run_async` 函数中埋点，Span 名称为 `invoke_agent {agent.name}`
- [调用模型时](https://github.com/google/adk-python/blob/v1.17.0/src/google/adk/flows/llm_flows/base_llm_flow.py)：在 BaseLlmFlow 的 `_call_llm_with_tracing` 函数中埋点，Span 名称为 `call_llm`
- [调用工具时](https://github.com/google/adk-python/blob/v1.17.0/src/google/adk/flows/llm_flows/functions.py)：在 BaseLlmFlow 的 `_execute_single_function_call_async` 函数中埋点，Span 名称为 `execute_tool {tool.name}`
...
<Callout type="info" title="埋点字段注意事项">
