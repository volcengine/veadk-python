...
Trusted MCP 在标准 MCP 之上扩展组件间的身份证明与端到端加密通信，防止服务身份伪造、数据篡改与流量劫持；结合机密计算（如 Jeddak AICC）和可信推理（如机密豆包），可让 Agent 与大模型运行在可信环境中。VeADK 通过 `tools` 机制原生连接 Trusted MCP 服务。这是[安全概述](/cn/docs/framework/security)中的可信执行平面。
