---
title: "KnowledgeBase 介绍"
description: "VeADK 知识库的功能、使用方法，以及 VikingDB 与 OpenSearch 两种后端的开通与配置。"
---

## 主要功能

KnowledgeBase 在 veadk 框架中扮演 Agent 的外部知识库角色。它像一个可供 Agent 随时查阅的、专门存储静态资料的“图书馆”。其核心功能包括：

1. **知识注入 (Ingestion)**：您可以将外部的、非结构化的文本资料（如产品文档、FAQ、文章、知识片段等）添加进知识库。框架会自动处理这些文本，将其转换为 Agent 可以理解和检索的格式。
   - 支持从文件导入 (`kb.add_from_files([...])`)。
   - 支持从内存中的文本字符串导入 (`kb.add_from_text([...])`)。
...
2. 点击创建知识库按钮，点完后会有一个弹出问是否导入文档，选择暂不导入。
...
[火山引擎 tos 文档](https://www.volcengine.com/docs/6349?lang=zh)
...
把上面资源开通部分，对应的内容 vikingDB、TOS 和 OpenSearch 对应的内容填入。注意 agent 的 api_base。

api_key 可以直接使用例子中的。access_key 和 secret_key 必须使用开通资源账号的。
...
下图红框里面显示了通过搜索知识库获取相关信息，然后大模型调用 calculate_date_difference function call。

![执行过程](/assets/images/knowledge/image-18.png)

下图显示 Agent 先调用 knowledge 获取相关知识，然后再通过 Function Call 调用 function calculate_date_difference。
...
在项目目录下新建 tmp 目录，在 tmp 目录新建 demo.txt。它的内容如下：
