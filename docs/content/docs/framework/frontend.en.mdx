...
- **New-chat modes**: retain the Agent conversation path, expose the upcoming temporary-session mode, or generate two real Skill candidates in isolated AgentKit CodeEnv sessions for comparison, ZIP download, and AgentKit publishing.
...
- **Insight Sandbox**: start an isolated temporary AgentKit Sandbox from the
  new-chat composer or global header. Studio reuses its dedicated CodeEnv tool,
...
Configure a dedicated AgentKit Tool in the `Ready` `CodeEnv` state before
starting the server:
...
write its result into the dedicated CodeEnv Tool.
`AGENTKIT_SANDBOX_TOOL_ID` is accepted as a compatibility fallback. Adding a
...
| Environment variable | Default | Description |
| :-- | :-- | :-- |
| `VEADK_SKILL_CREATOR_TOOL_ID` | — | Dedicated AgentKit CodeEnv Tool ID. |
...
For a cloud deployment, pass `--skill-creator-tool-id`. The deploy command uses
the deployer's credentials to obtain the Ark key, stores it in KMS through
AgentKit credential hosting, and binds only the relay URL and revocable ticket
to the dedicated CodeEnv Tool. The VeFaaS Function receives no model
credential:
