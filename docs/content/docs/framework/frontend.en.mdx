...
`.github/workflows/publish-studio-release.yaml` runs only after relevant Studio
changes reach `main`. GitHub checks out the exact SHA, builds the frontend,
verifies the fixed offline wheels, and uploads a prepared source archive through
a short-lived job-bound TOS URL. The API-key-protected VeFaaS release server
consumes and deletes that private object, builds and publishes the immutable
Bundle and Manifest, updates the bounded `releases.json` catalog, and replaces
`latest.json` last. Store only `STUDIO_RELEASE_SERVER_URL` and
