...
`--admin` and `--developer` each accept a comma-separated list of local
usernames or OAuth email addresses. Whitespace is ignored and matching is
case-insensitive. If the same identity appears in both lists, `admin` takes
precedence. For a local Studio:
...
The equivalent environment variables are `VEADK_STUDIO_ADMINS` and
`VEADK_STUDIO_DEVELOPERS`.

Use the same options for a deployed Studio:
...
The account footer shows the OAuth email below the display name. Purple,
yellow, and green badges identify admins, developers, and regular users.
