...
revocable gateway ticket. Model credentials are never returned to the browser,
and the Skill creation APIs require the Studio developer or admin role.
...
Candidate sessions have a 30-minute TTL. Job state is stored in Sandbox rather
than frontend-process memory, so polling and downloads continue to work when
FaaS requests reach different instances.
