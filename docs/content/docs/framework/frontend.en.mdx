...
- **Custom-agent workbench**: debug an agent beside its configuration. Initialization, session, and conversation failures show credential-redacted error details and Runner logs that can be expanded and copied. Then review generated source with line numbers and syntax highlighting and set the region, message channel, network, and environment variables before deploying to AgentKit. The global task list shows the actual AgentKit-generated Runtime name, region, and progress across pages; in-flight tasks can be cancelled and cleaned up, and failed tasks can be retried. Remote topology and traces are fetched through the selected Runtime data plane.
