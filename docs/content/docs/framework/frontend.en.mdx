...
- Selecting **Code execution** in the custom Agent's built-in tools adds the
  ...
  channel, network, and environment settings remain primary on the right. The
  global task list shows the actual AgentKit-generated Runtime name, region,
  and progress across pages; in-flight tasks can be cancelled and cleaned up,
  and failed tasks can be retried. Remote topology and traces are fetched
  through the selected Runtime data plane.
