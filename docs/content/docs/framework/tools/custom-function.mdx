---
title: "自定义工具"
description: "在 VeADK 中把你自己的 Python 函数封装成工具：普通入参函数、携带运行时上下文的函数、长时运行任务。"
---

当内置工具无法满足业务需求时，你可以把任意 Python 函数封装成**自定义工具**，扩展智能体的能力。本页介绍三种形式：普通入参函数、携带运行时上下文的函数，以及长时运行任务。
...
<Cards>
  <Card title="内置工具" href="/cn/docs/framework/tools/builtin" description="网页搜索、图像 / 视频生成、代码沙箱等。" />
  <Card title="内置 MCP 工具" href="/cn/docs/framework/tools/builtin-mcp" description="飞书 / Lark、LAS、视频云、MCP Router 等。" />
</Cards>
