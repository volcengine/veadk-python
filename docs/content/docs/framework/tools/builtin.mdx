---
title: "内置工具"
description: "VeADK 内置的原生函数工具：网页搜索、向量检索、网页读取、图像 / 视频生成、代码沙箱、手机操作、语音合成等。"
---
...
<Callout type="warn">
内置工具依赖火山引擎相关服务。使用前请开通对应服务，并完成 AK/SK、API Key 等鉴权配置（见每个工具的“环境变量”说明），详见[配置](/cn/docs/framework/configuration)。
</Callout>
...
<Callout type="info">
模型与鉴权配置在多个工具间通用。本页只在第一个用到的工具处完整展示一次 `config.yaml`，后续工具仅列出新增的配置项。完整配置说明见[配置](/cn/docs/framework/configuration)。
</Callout>
...
<Cards>
  <Card title="内置 MCP 工具" href="/cn/docs/framework/tools/builtin-mcp" description="飞书 / Lark、LAS、视频云、MCP Router、Supabase 等。" />
  <Card title="自定义工具" href="/cn/docs/framework/tools/custom-function" description="把你自己的函数封装成工具。" />
  <Card title="系统工具" href="/cn/docs/framework/tools/system-tools" description="知识库与长期记忆的自动挂载工具。" />
</Cards>
