...
description: "VeADK's native function tools: web search, vector retrieval, link reading, image / video generation, code sandboxes, mobile control, and text-to-speech."
---
...
<Callout type="warn">
Built-in tools depend on Volcengine services. Enable the corresponding service and configure authentication (AK/SK, API keys — see each tool's "Environment variables" notes) before use. See [Configuration](/en/docs/framework/configuration).
</Callout>
...
<Callout type="info">
Model and authentication settings are shared across many tools. This page shows the full `config.yaml` once, at the first tool that needs it; later tools list only the additional keys. See [Configuration](/en/docs/framework/configuration) for the full reference.
</Callout>
...
<Cards>
  <Card title="Built-in MCP tools" href="/en/docs/framework/tools/builtin-mcp" description="Feishu / Lark, LAS, VOD, MCP Router, Supabase, and more." />
  <Card title="Custom tools" href="/en/docs/framework/tools/custom-function" description="Wrap your own functions as tools." />
  <Card title="System tools" href="/en/docs/framework/tools/system-tools" description="Auto-mounted tools for knowledge base and long-term memory." />
</Cards>
