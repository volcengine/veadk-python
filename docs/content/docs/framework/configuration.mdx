---
title: "配置"
description: "VeADK 的配置优先级、环境变量对照表、内置常量及运行时动态配置。"
---

VeADK 从环境变量、`.env` 与 `config.yaml` 中读取配置项，让你把密钥与连接信息从代码中分离，实现代码脱敏。本页是配置项的完整参考。
...
在 VeADK 中，某些值如果没有显式指定，且环境变量中也不存在时，如下值将会取默认值：

| 名称 | 值 | 释义 |
| :- | :- | :- |
| DEFAULT_AGENT_NAME | `veAgent` | Agent 的缺省名称 |
| DEFAULT_MODEL_AGENT_NAME | `doubao-seed-1-8-251228` | Agent 的推理模型名称 |
| DEFAULT_MODEL_AGENT_PROVIDER | `openai` | Agent 的推理模型提供商，方舟平台兼容 openai 规范，因此使用方舟默认是 openai 即可 |
| DEFAULT_MODEL_AGENT_API_BASE | `https://ark.cn-beijing.volces.com/api/v3/` | 模型 API 基础地址 |
| DEFAULT_IMAGE_GENERATE_MODEL_NAME | `doubao-seedream-5-0-260128` | image_generate 工具默认使用的方舟生图模型 |
| DEFAULT_IMAGE_GENERATE_MODEL_API_BASE | `https://ark.cn-beijing.volces.com/api/v3/` | image_generate 工具默认使用的方舟生图模型 API 基础地址 |
| DEFAULT_VIDEO_MODEL_NAME | `doubao-seedance-2-0-260128` | video_generate 工具默认使用的方舟视频生成模型 |
...
除环境变量与常量外，VeADK 还支持在运行时对智能体配置进行动态更新（热更新），由 `DynamicConfigManager` 统一管理。动态配置采用 JSON 结构，其中包含一个 `agent` 数组，可对单个或多个智能体的字段进行覆盖。

每个智能体可配置的字段如下：

| 字段 | 释义 |
| :- | :- |
| `id` | 必须与目标 Agent 的 ID 一致 |
| `name` | 用于展示与日志记录的名称 |
| `description` | 用于展示与日志记录的描述 |
| `model_name` | 在火山引擎注册的模型名称 |
| `instruction` | 定义 Agent 行为与能力的提示词 |
...
<Cards>
  <Card title="安装" href="/cn/docs/framework/getting-started/installation" description="配置 API Key，跑通环境准备。" />
