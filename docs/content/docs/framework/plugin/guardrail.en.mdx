...
description: "The VeADK content_safety guardrail tool, built on the Volcengine LLM Application Firewall, audits content across the agent lifecycle."
---
...
<Callout type="info" title="Roadmap: agent-level callbacks">
`content_safety` also exposes `before_agent_callback` and `after_agent_callback`, but these are placeholder implementations today (they return `None` and perform no checks) and are not yet active. Agent-level input validation and context analysis are planned; for now you don't need to wire up these two callbacks.
</Callout>
...
<Cards>
  ...
  <Card title="Create an agent" href="/en/docs/framework/agent/create" description="Learn about agent callbacks and the lifecycle." />
</Cards>
