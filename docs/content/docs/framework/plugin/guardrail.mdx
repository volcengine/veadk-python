---
title: "护栏工具"
description: "VeADK 基于火山大模型应用防火墙的内容安全护栏工具 content_safety，对 Agent 生命周期的各阶段进行内容审计。"
---

`content_safety` 是 VeADK 基于 Agent 插件机制提供的内容安全护栏工具：它挂到 Agent 执行流程的回调上，借助[火山大模型应用防火墙](https://www.volcengine.com/product/LLM-FW)对各阶段内容做合规审查，拦截不安全的输入与输出。
...
<Callout type="info" title="路线图：Agent 级回调">
...
<Cards>
  ...
  <Card title="构建智能体" href="/cn/docs/framework/agent/create" description="了解 Agent 的回调与生命周期。" />
</Cards>
