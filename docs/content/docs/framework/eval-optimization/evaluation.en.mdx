---
title: "Evaluation"
description: "Learn the general evaluation workflow in VeADK, the evalset format, and how to use the ADKEval and DeepEval evaluators."
---
...
With the `veadk uploadevalset` command, you can easily upload a local evalset file to the CozeLoop platform.
...
This command uploads an evaluation dataset in Google ADK format from a JSON file to the CozeLoop platform for agent evaluation and testing.

**Parameters**

- `--file TEXT`: The path to the JSON file containing the dataset [required]
- `--cozeloop-workspace-id TEXT`: The CozeLoop workspace ID used to organize the evalset. If not provided, the `OBSERVABILITY_OPENTELEMETRY_COZELOOP_SERVICE_NAME` environment variable is used.
- `--cozeloop-evalset-id TEXT`: The specific evalset ID to upload to. If not provided, the `OBSERVABILITY_OPENTELEMETRY_COZELOOP_EVALSET_ID` environment variable is used.
- `--cozeloop-api-key TEXT`: The API key used to authenticate with the CozeLoop service. If not provided, the `OBSERVABILITY_OPENTELEMETRY_COZELOOP_API_KEY` environment variable is used.
