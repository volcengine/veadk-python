---
title: "Tunnel"
description: "Connect on-prem / private resource servers to a cloud agent through an outbound tunnel — no inbound port. The resource type is selected by a protocol; MCP is the first supported one and it's extensible."
---
...
```text
  ...
  resource server(s) ◀─ connector ─outbound WSS─▶ /tunnel/connect ─ registry
  (MCP / ...)            (registers to agent) │     agent + TunnelToolset
                                              │       (mounts tools per turn, per protocol)
```
