---
title: "VeADK 命令行工具"
description: "VeADK CLI 提供项目初始化、智能体创建、Web 调试、知识库、部署、评测等全流程命令。"
---
...
<Callout type="info" title="注意">
  ...
  - 代码中的'app_name'需要与上面的命令中的'app_name'保持一致，在本示例中为'app'。
</Callout>
...
<Callout type="info" title="注意">
  ...
  - 在部署过程中，您可以在火山引擎 [VeFaas 控制台](https://console.volcengine.com/vefaas/) 查看部署状态和日志，确保部署成功。
</Callout>

部署过程的详细日志如下图示：

![部署日志](/assets/images/cli/cli_veadk_deploy_vefaas_log.png)

部署成功后，您可以在火山引擎 [VeFaas 控制台](https://console.volcengine.com/vefaas/) 查看您部署的智能体应用。
...
<Callout type="info" title="注意">
  - 您需要先在火山引擎 [PromptPilot 控制台](https://promptpilot.volcengine.com/) 创建一个项目，并获取 API Key 和工作空间 ID。
  - 您也可以在项目根目录下的`.env`文件中设置 `PROMPTPILOT_API_KEY` 环境变量，避免在命令行中明文暴露 API Key。
  - 本命令会自动读取智能体代码中的 `Agent` 对象的 `instruction` 内容，作为目标来进行提示词优化。
</Callout>

本例中，原始的 `instruction` 内容如下所示：
...
<Callout type="info" title="注意">
  ...
  - 需要事先准备好评测数据集文件，您可以参考[评测](/cn/docs/framework/deploy/evaluation)中的说明，创建一个符合 Google ADK 格式的 JSON 文件。
</Callout>
...
VeADK 命令行兼容 AgentKit 命令，您可以直接在 AgentKit 相关命令前添加 `veadk` 前缀。详情请参阅 [AgentKit 命令兼容](/cn/docs/cli/agentkit-cli)。
