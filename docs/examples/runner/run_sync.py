...
class HelloAgent(BaseAgent):
    async def run_async(self, ctx: InvocationContext):
        ...
        event = Event(
            author=self.name,
            content=types.Content(parts=[types.Part(text=f"今天是个晴天，温度15~22摄氏度，非常适合户外活动！")])
        )
...
async def main():
    ...
    await session_service.create_session(user_id=USER_ID, app_name=APP_NAME, session_id=SESSION_ID)
