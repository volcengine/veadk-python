...
- **FR-5 — Runtime env injection.** All resolved configuration is set as VeFaaS function environment variables (create + release), matching the keys `mpa-agent` reads at startup (`.env.example`, `docs/mpa-agent-api.md` §9). This includes `IDENTITY_STARTUP_ENABLED=false` (FR-10) and `MPA_CODEX_WORKER_ENDPOINT_PREFERENCE`/endpoint choice consistent with the public-endpoint decision (FR-11).
...
- **FR-9 — Non-regression of `mpa-agent`.** No change to `mpa-agent` source or image is required or performed; the runtime's existing REST session API, A2A server, MCP server, Codex sandbox delegation, scheduled tasks, and IM channels behave exactly as in the current image.
- **FR-10 — Identity adaptation (no arkclaw identity resources).** The veadk scenario does not use `arkclaw-{space}-userpool/-client/-workload`. The command injects `IDENTITY_STARTUP_ENABLED=false` so `initialize_identity_config_startup` short-circuits (`app/identity/service.py:166`), and derives `CLAW_SPACE_ID` from the account id via a stable mapping (e.g. `csi-<account_id>`), satisfying the runtime's non-empty `CLAW_SPACE_ID` requirement without provisioning identity pools. `MPA_AGENT_ID` remains a required `mi-*` parameter.
...
- Open blockers: none.
- Executed checks: none yet (implementation not started).
- Remaining scope: implementation tasks T-1..T-9 (+ T-3b), test-first per `AGENTS.md`.
