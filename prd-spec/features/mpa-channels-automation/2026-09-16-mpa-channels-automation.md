...
### Upstream rebase and PR verification (2026-09-18)

The user authorized rebasing the feature branch onto `superops-team/veadk-python` and opening an upstream PR. Rebased the three feature commits onto `main` at `34658d75`, retaining upstream MPA runtime-filter performance changes. Preserved the original local history in `backup/mpa-channels-before-upstream-rebase-20260918`. Conflicts affected generated `veadk/webui` assets only; source merged cleanly and the final app/widget assets were rebuilt from the combined source. No component contract changes were introduced by the rebase.

Verification scope: rebased source at `63e6c9bd` plus regenerated WebUI assets and this bilingual verification record. Checks ran on 2026-09-18:

- **pass**: `npm --prefix frontend test` — 1207 tests.
- **pass**: `./frontend/node_modules/.bin/vitest run --root frontend --environment jsdom tests/mpaChannelsAutomation.test.tsx tests/runtimeChannels.test.tsx src/adk/channels.test.ts --maxWorkers 1` — 65 tests. An initial run without the browser environment failed the API test because `window` was unavailable; the explicit jsdom run passed.
- **pass**: `uv run --extra dev pytest tests/test_mpa_channel_proxy_policy.py tests/cli/test_frontend_runtime_proxy.py tests/integrations/test_mpa_provision_env.py -n 2` — 89 tests; two workers used for the affected proxy/deployment regression scope. The initial invocation used an incorrect provisioning-test filename and collected zero tests; only the corrected run counts as verification.
- **pass**: frontend i18n consistency, production app/widget build, and `test:webui-assets` — 104 packaged files and 248 references.
- **pass**: `uv run --extra dev pre-commit run --all-files` — Ruff checks, formatting, and repository secret scanning.
- **fail (upstream baseline)**: Pyright on `veadk/cli/cli_frontend.py`, `veadk/integrations/mpa/channel_proxy.py`, `veadk/integrations/mpa/mpa_provision.py`, and `tests/test_mpa_channel_proxy_policy.py` reports 36 errors, all in `cli_frontend.py`. The upstream `34658d75` version has exactly the same diagnostic severity/rule/message multiset: zero added or removed diagnostics. Pyright was supplied temporarily with `uv run --with pyright pyright --pythonpath .venv/bin/python`; project dependencies and global configuration were unchanged.
- **pass**: isolated browser smoke verifies compact creator/relative-time metadata, region/Runtime ID, Chinese search and keyboard selection, default quick configuration, switching to Feishu manual credentials, and the 390×844 layout (document width and scroll width both 390). APIs used synthetic fixtures; temporary fixture files, tab, viewport override, and server were cleaned up.
- **not_run**: live provider/cloud operations and full Python repository regression; this rebase retained production source behavior, and affected proxy/deployment tests were run. Prior full interaction coverage remains documented above.

Remaining limitation: the upstream Python typing baseline is not clean. The PR must disclose it rather than report Pyright as passing.
