...
- 状态：`approved`；实现与浏览器验证已完成至 S5-12，S5-13 至 S5-15 证据门禁待完成
- 创建 / 修订：2026-09-15 / 2026-09-20
...
本期完成 P0 核心链路：创建/更新 MPA Agent、Runtime 准备、Profile 应用、Session、对话和 worker 执行、会话执行配置、暂停/恢复/续跑、历史结果、Debug/Trace、运行时更新与故障恢复。当前 VeADK/Runtime 主干已由其他 feature 独立交付消息渠道绑定、飞书/钉钉/企微配置、shared APIG 部署、持久 Runtime/数据库部署、Skill Space 部署和完整 artifact answer replay；这些能力可作为集成基础，但渠道/provider 专项验收仍归 MPA 渠道与部署契约，不替代本 P0 的 AC/VC 门禁。网站集成、资源库和全局搜索仍不属于本 P0。
...
| `POST /api/v1/sessions/{sessionId}/profile-upgrade` | 新增 | body 含 `targetProfileRevision`，Header 含当前 execution-config `If-Match` 与 `Idempotency-Key`；目标 Profile 必须已 applied。活动 Turn 可提交但仅下一 Turn生效；版本回退返回 `409 downgrade_not_allowed`。Runtime 将 request hash 与安全结果持久化到 SQL `runtime_operations`；相同 key/相同目标可跨 Runtime 进程 replay，相同 key/不同目标返回 `409 idempotency_mismatch` |
...
Runtime operation：`accepted -> running -> succeeded | failed_retryable | failed_terminal`。Profile apply 当前在接受后同步完成，因此通常直接从 `accepted` 进入 `succeeded` 或 `failed_terminal`；其对外成功结果仍为 `status=applied`。`failed_retryable` 可重新进入 `running`，终态不能改写为另一终态。状态更新使用 compare-and-swap，避免并发完成互相覆盖。
...
删除状态机：`preview -> blocked | ready -> cleaning_sessions -> deleting_runtime -> deleted | failed_retryable`。当 MPA operation store 中存在真正 `active` 的 create/update operation、Runtime binding 缺失或重复、Runtime Profile orphan，或 Runtime Session 处于 `queued|running|pausing|paused|resuming` 时，删除在任何 mutation 前进入 `blocked`。如果某条遗留 operation 为 `failed_retryable`，但 Runtime binding 已是 `bound` 且 `profile-status` 返回 `applied`，说明 Runtime 已收敛，不应继续阻止用户清理资源。`cleaning_sessions` 只删除可见 idle/terminal Runtime Session，并把单个 Session 的 `404` 视为已清理。`deleting_runtime` 只在 Runtime 侧清理成功后调用 AgentKit Runtime delete API。
...
正式 migration 按 `runtime_operations -> mpa_agents/mpa_meta/session_meta 增量列 -> session_execution_configs -> turn_execution_records -> a2a_turn_participants -> Secret reference 列/索引 -> 其余索引/约束` 顺序执行，并保存 schema version。部署先扩 schema、再上线双版本可读代码、最后启用新写路径；旧代码无法安全读取新状态时禁止回滚。AgentKit mode 禁止新写 `agent_api_key/model_api_key` 明文；现有新平台值转为受保护引用后清空。S5-01a 还会在迁移读到 legacy 明文后、任何 provider 调用或诊断输出前，把这些明文注册到进程内 value-aware redaction registry。API access log、Session 对用户可见输出、Runtime Console 响应、lifecycle details、trace 持久化以及 `runtime_operations.response_snapshot/resource_ids` 均经过同一脱敏边界。Runtime operation 状态响应不返回 principal、幂等键或 request hash。
...
- execution-ready smoke：`POST /api/v1/readiness/execution-smoke` 不依赖模型选择工具。Runtime 先执行最小模型 prompt（禁止工具）并校验模型终态，再通过现有 Worker API 由 `CodexWorkerClient.create_session/start_turn/get_turn` 发起隔离 Worker Turn `printf mpa-smoke-<operationId>`，并用 `follow=false` 回放最终事件；幂等键由 creation operation ID 派生，最多重试 1 次，总超时 120 秒。Worker `/v1/codex-worker/ready` 必须返回 `ready=true`；Worker Turn 必须到达终态并返回可读 replay events。marker 匹配仅作为诊断字段 `markerObserved`，因为 worker 可能汇总命令输出。失败按阶段分类，例如 `smoke_model_failed`、`smoke_worker_not_ready`、`smoke_worker_turn_wait_failed`、`smoke_worker_events_failed`，并在安全可用时附带 `sandboxStage`、HTTP status、worker phase、tool API action 和 provider code。Session/workspace 清理成功后才进入 `runnable`；清理失败为 `failed_retryable`，失败证据最多保留 24 小时。
...
| 2026-09-16 | S5-01a canary-safe value-aware redaction | pass | Runtime 现在会在 Secret reference migration 中注册 legacy 明文 Secret 字面值，并在 API access log、Session 可见文本/events、Runtime Console 响应、lifecycle/trace detail bounding、SQL trace-step 持久化及 Runtime operation 的 response/resource snapshot 中执行 value-aware redaction。该策略可以在非敏感 key 或自由文本中遮盖已注册 canary 值，同时保留 `flow_id`、`user_code`、模型 `max_tokens` 等非 Secret 业务字段；Secret reference 字段允许保存。Runtime operation 状态不暴露 principal 或幂等内部字段。最新验证见下方 2026-09-17 Runtime operation/CAS 记录。 |
...
| 2026-09-18 | S5-09 Studio 控制面 CLI | 通过，Pyright 除外（blocked） | 新增独立的 `veadk mpa control` 命令组，同时保留基础设施用途的 `veadk mpa create`。它仅通过共享 `MpaControlPlaneClient` 和 Studio BFF 提供 view/create/update、operation list/get/retry、Profile status/apply、Session config get/patch/profile-upgrade 和 delete preview。JSON 文件/stdin 输入使用共享 Pydantic schema；Profile Secret 字段在发网前失败；bearer 从不输出；写操作保留显式、由调用方稳定复用的幂等键和 CAS revision/ETag；响应 ETag 保留；脱敏后的控制面失败映射到固定退出码 `1/3/4/5`，本地用法错误继续使用 Click 退出码 `2`。loopback BFF 证明了真实请求路径、Header、body、202 处理、旧 Runtime 只读输出，以及 BFF 已收到请求后的同 key timeout 重放。两轮审查修复了嵌套 help 在未配置 URL 时不可用的问题，并澄清 JSON 与本地 usage-error 文本的边界。验证：focused CLI 22 passed；共享 client 加 MPA CLI 57 passed；全量 CLI 1402 passed、4 skipped；规定的并行 Python 回归 4661 passed、8 skipped、2 xfailed；focused Ruff/format、compileall 和 diff check 通过。Pyright 因环境没有可执行文件记为 `blocked`。无需修改 Runtime API、Runtime 数据库、前端、Managed Agent 或 `agentkit-mpa-agent`。 |
| 2026-09-18 | S5-10 固定跨仓版本的兼容性预检 | pass | 使用 VeADK `486dc06e429573ce43d6462b6dc960be4bc34ab6`、Runtime `eecc6e3115685e5cb86dcfbff4fb7c6ac7a10dee`、AgentKit SDK `0.8.5` 和已部署 v62 镜像 digest `sha256:6ac6a2712ecd1c7950125dc9afc6467373a08142fbd6c3577aba12f126079bef` 执行 VC-19 预检阶段。实时 Runtime detail 查询返回 `Ready`、version 62 和预期 artifact tag；registry 查询独立返回相同 digest。真实 manifest 命中 `p0-codex-rest-v1`。负向执行中，`codex-v2` 以 exit 3 / `worker_protocol_incompatible` 被拒绝，非法镜像 digest 以 exit 2 / `invalid_runtime_image_digest` 被拒绝。manifest 契约测试 4 passed，Studio/GitHub 副作用前阻断测试 8 passed，VC-15 Runtime 脱敏测试 91 passed。脱敏证据位于 `evidence/contract/s5-10-v62-20260918/`。S5-10 为只读检查，不需要修改代码、Runtime schema、API、前端、Managed Agent 或线上资源。VC-19 的最终全量回归、覆盖率、PostgreSQL、构建和性能阶段仍由 S5-11 之后的 S5-13 执行。 |
| 2026-09-18 | S5-11 真实 Runtime 生命周期、回滚与删除 | pass | VC-21 使用全新隔离 Runtime 执行，镜像为 v29 `agentkit-platform-2112682748-cn-beijing.cr.volces.com/agentkit/mpa_agent_vc21c:mpa-p0-smoke-v29-eecc6e3-20260918`，digest 为 `sha256:078519c796b03b298a95ef7a6614f92121bc124513dbb472a7496c95821ee723`。run `vc21-20260918-045` 完成全部阶段：兼容性栅栏、Runtime 创建、Profile apply、execution-ready smoke、同镜像 no-op update、受控失败和恢复 smoke、兼容 rollback smoke、active operation 删除阻断、active session 删除阻断、idle session 清理、最终 Runtime 删除和 PostgreSQL 清理。runner 返回 `{"case":"VC-21","residue":[],"status":"passed"}`。真实验证中 `vc21-20260918-044` 暴露一个 Studio 删除 bug：Runtime Profile 已 `applied` 且 smoke 已通过时，遗留的 `failed_retryable/smoke_running` operation 仍可能被视为永久 active blocker；Studio 现改为将该已恢复状态视为非阻塞，同时继续保留真正 `active` operation 与 active session 的阻断。修复后的定向 CLI/route/driver 覆盖返回 49 passed，focused Ruff 通过。证据位于 `evidence/live/vc21-20260918-045/`。 |
| 2026-09-19 | S5-12 全矩阵浏览器阶段执行 | partial；`BC-03/04/05/06/09` pass，`BC-01/02/07/08` blocked | 使用 gstack v1.87.4.0 驱动 system Google Chrome，对 VeADK `cd34b45c` 加当前 working diff 的 loopback fixture 执行真实浏览器验证。`BC-03` 覆盖 `bound`、`runtime_missing`、`binding_ambiguous`、`orphan_runtime` 和旧 Runtime 只读状态。`BC-04` 使用两个隔离 Chromium profile，以相同本地 principal 和 ETag `"6"` 打开同一 Session：浏览器 A 的 model PATCH 返回 `200`/revision 7，浏览器 B 的过期 MCP clear 返回 `412`，权威状态只保留 A 的模型覆盖。`BC-05` 将 Session Profile 从 6 升级到 7，并证明同 key replay 与不同目标的 `409 idempotency_mismatch`。`BC-06` 在发送后和刷新后都只显示一次持久事件与续传事件，未知 cursor 返回 `410 cursor_expired`。`BC-09` 证明 `running -> pausing -> paused -> running`、partial ACK、已本地化的 `new_turn_required`、一次显式 `/continue` 加 `/sse`、刷新后不自动续跑，以及同 key linked Turn replay。执行中修复了 test-only MPA view、Session list/events、cursor 终态持久化和可控生命周期流接线，限制 Vite 依赖发现只扫描产品入口，并移除了分支集成后导致 TypeScript build 失败的 3 处重复 `agentCategory` 声明/字段。验证通过：scenario/Profile route pytest 36 passed、focused frontend tests 82 passed、完整 frontend tests 1240 passed、Ruff pass、i18n pass、frontend production build pass（仅有既有 chunk warning）。`npm ci` 仍为 `fail`，因为 `frontend/package.json` 与 `frontend/package-lock.json` 的 esbuild 0.28 依赖树不一致；本次 build 仅修复本地依赖目录，未修改 lockfile。`BC-01`、`BC-02`、`BC-07` 与 `BC-08` 为 `blocked`：`create_success`、四种创建失败变体和 `slow_scope_switch` 没有隔离的 scenario 写/scope 路由，而真实创建 UI 会调用 `/web/deploy-agentkit`，因此没有执行未经授权的云资源变更。`evidence/browser/s5-12-20260919-r1/` 下的本地证据按要求不提交。`S5-12` 与 `VC-17` 仍保持未完成。 |
| 2026-09-20 | S5-12 完整浏览器矩阵 | pass；`BC-01`–`BC-09` 全部通过 | loopback-only harness 已隔离 Runtime 名称校验、生成项目/草稿、部署 SSE、MPA operation create/list/get/retry、失败阶段、部署/operation barrier，以及 principal/region scoped Runtime discovery，因此不可能触达 AgentKit SDK 或云资源写操作。`BC-01` 通过正常创建，以及从 `profile_applying` 刷新恢复到 `runnable`；`BC-02` 覆盖 `profile_apply_failed`、`smoke_worker_not_ready`、`smoke_output_mismatch`、`cleanup_failed`，每种场景均只有一次 Runtime deploy、一次 operation create 和一次显式 operation-only retry；`BC-07` 证明 composition 中 Enter 调用数为 `0`、compositionend 后为 `1`，且 `1024x768` 下无水平溢出；`BC-08` 在同一 Runtime ID 下延迟 Alice/北京响应后仍保持 Bob/上海为权威页面状态，denied principal 只得到 `404 resource_not_found`。浏览器执行发现并修复：前端缺少 active operation 刷新恢复、快速/传统创建入口的失败 retry 会重跑完整部署、IME 防误提交缺失、恢复成功文案未翻译、retry rejection 未处理，以及 active/failed-retryable operation 期间 `canDebug` 仍为 true。当前 working diff 最终检查：frontend 1243 passed、focused MPA backend 53 passed、TypeScript pass、focused Ruff pass、i18n pass、production build pass（仅既有 chunk warning）、diff whitespace pass。`npm ci` 仍为此前记录的 dependency-lock failure，归 S5-13 处理。本地浏览器证据继续位于 `evidence/browser/s5-12-20260919-r1/`，按要求不提交。 |
| 2026-09-20 | 本地 Studio Profile 页错误处理补救 | pass | 用户在 `http://127.0.0.1:5179/` 验证时发现 Profile 页仍有报错。根因拆分为三类：本地后端此前未带 `--dev` 启动，未配置 Studio TOS 时 MPA operation repository 为空，导致 `GET /web/mpa/agent-operations?status=active` 返回 `503 mpa_operation_service_unavailable`；当前选中 Runtime `r-yev5cmzp4wzn6n5iodgd` 为 version 14，仍要求旧 `X-Jwt-Token`，因此原生 Session list 返回上游 `401`；其 Profile status endpoint 也返回普通 `404 Not Found`。Studio 已改用 `--dev` 重启；BFF/前端现在把旧 Runtime auth/profile-status 失败归一为稳定 safe error 和本地化只读提示，同时继续保持 Runtime 写入和 Debug fail-closed。验证：后端 Profile route tests 28 passed，前端 focused source-contract run 1244 passed，i18n pass，本地 operation list 返回 `200 {"operations":[]}`，旧 Runtime Profile view 返回 `200` 且 `safeError.code=profile_not_found`；Session list 在该 Runtime 升级前仍是预期的上游 `401 X-Jwt-Token header is required`。见 bugfix PRD [studio-mpa-profile-errors](../../bugfixes/studio-mpa-profile-errors/2026-09-20-studio-mpa-profile-errors.zh.md)。 |
...
2026-09-18：使用固定双方仓库 revision 和线上 v62 registry digest 完成 S5-10 兼容性预检。已批准组合命中 `p0-codex-rest-v1`；不支持的协议和非法 digest fixture 均在任何 mutation 路径前按约定退出码失败。线上 Runtime 还被独立核验为 version 62 且状态 `Ready`，VC-15 脱敏测试保持通过。本次只完成 VC-19 的 preflight 部分；S5-13 仍在 S5-11 之后负责最终回归、覆盖率、PostgreSQL、构建与性能门禁。

2026-09-18：完成 S5-11，真实 VC-21 run 为 `vc21-20260918-045`。该隔离 run 基于 source Runtime v62 创建独立 AgentKit Runtime、Tool 和 PostgreSQL database，应用 Studio Profile，通过真实 Worker 协议完成 execution-ready smoke，验证同镜像 no-op update/recovery、带 `noRollbackNeeded=true` 的兼容 rollback smoke、active operation 与 active session 删除阻断、通过 Runtime API 清理 idle Session，最后删除 AgentKit Runtime 与 PostgreSQL database，最终 `residue=[]`。上一轮真实 run 暴露出一个问题：Profile 已 applied 后，遗留的 `failed_retryable/smoke_running` operation 仍会永久阻塞删除；Studio preview/delete 现已把该已恢复状态视为非阻塞，同时继续保留真正 active operation 和 active Session 的阻断。
