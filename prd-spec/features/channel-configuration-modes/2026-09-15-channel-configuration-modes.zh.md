...
### 视觉修正（2026-09-15，用户明确要求）

用户要求三个渠道统一使用“扫码绑定”、配置方式选中态清晰、群会话策略框改色。正式 Studio 入口未加载组件预览的主题变量，上轮隔离预览额外引入主题导致漏检。采用 Radio 公开 label 插槽展示页面所属的选中/未选中颜色，Select 公开 style 属性使用当前页面输入框的背景、文字和边框色，不覆盖组件内部选择器、不全局注入主题。统一复用 `channels.bind` 文案。直接评审通过：这是 FR-5 / AC-4 的视觉修正，用户当前请求授权实施；运行时、API、绑定生命周期和组件公开接口无变化，CON-15 无需修改。验证使用与正式入口相同的 CSS，不额外注入组件主题。提交/部署不属于本次新增请求。

视觉修正验证（2026-09-15，基于 `f5f5ab1e` 的本轮 diff）：pass，测试先出现 7 项失败，修正后渠道测试 49 项通过；`npm --prefix frontend test` 1208 项通过；`npm --prefix frontend run check:i18n`、生产构建和构建资源验证通过（104 文件 / 248 引用）。真实浏览器未加载组件主题变量，实测选中项 `rgb(9, 9, 11)` / 600，未选中项 `rgb(113, 113, 122)` / 400；选择框白色背景、深色文字、浅色边框。点击和键盘切换有效，企微显示“扫码绑定”，390px 视口无横向溢出。临时预览已清理。Python、真实消息收发及全量 pre-commit：not_run，本轮仅视觉和文案修正且未请求提交。
