...
### Visual correction (2026-09-15, explicitly requested)

The user requests the same Bind with QR code label for all providers, visible configuration-method selection, and a lighter group-scope field. The main Studio entry does not load component-preview theme tokens; the previous isolated preview added those tokens and missed this context. Use the Radio public label slot for page-owned selected/unselected text colors, and the Select public style prop for the existing page input background, foreground and border. Do not override component internals or inject a global theme. Reuse `channels.bind`. Direct review passed: this is a visual correction to FR-5 / AC-4 authorized by the current user request; runtime, API, binding lifecycle and public component interfaces remain unchanged, so CON-15 needs no update. Verification must use the production entry CSS without extra component-theme imports. Committing/deploying is outside this new request.

Visual correction verification (2026-09-15, current diff on `f5f5ab1e`): pass, test-first run had 7 failures; after correction, 49 channel tests passed. `npm --prefix frontend test`: 1208 passed; `npm --prefix frontend run check:i18n`, production build and built-asset verification passed (104 files / 248 references). Real browser without component theme tokens measured selected text `rgb(9, 9, 11)` / 600 and unselected text `rgb(113, 113, 122)` / 400; the Select has a white background, dark text and light border. Click/keyboard selection works, WeCom shows the shared QR binding label, and the 390px viewport has no horizontal overflow. Temporary preview removed. Python, live messaging and full pre-commit: not_run, because this iteration changes only visuals/copy and no commit was requested.
