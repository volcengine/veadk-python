...
`GET /web/runtimes` keeps its response shape and adds an optional query parameter `agentCategory`. Invalid values return `400`. Runtime category is derived only from explicit Runtime tags. When a category is present, each regional scanner keeps reading control-plane pages until it has enough matching items for the requested merged page or the remote cursor ends. The response still includes `agentCategory` on every item.
