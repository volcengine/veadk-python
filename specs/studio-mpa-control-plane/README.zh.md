...
- `CON-16`: 本期复用 Studio 现有创建表单和 AgentKit 平台资源，补齐 MPA Runtime 部署、Profile 应用、会话执行配置、会话结果与 Debug；不使用 Managed Agent CRUD/version/Session，不为 MPA 新建模板/专家/审核安装产品。消息渠道绑定与配置已由主干按独立 `mpa-channels` 契约交付，Runtime 主干也已具备 shared APIG、Runtime/数据库和 Skill Space 部署支撑；其 provider 专项验收不计入本 P0 台账。网站集成、资源库和搜索在本 P0 中仍延后；不导入历史 ArkClaw 数据。
...
- `CON-21`: 本期交互/Debug 执行委托 runtime service，保证授权与隔离。主干渠道管理仅通过独立 Automations 与 Runtime proxy 契约消费；Runtime ADK stream 已支持完整 artifact answer replay。本 P0 不重复建设这些产品面，也不把渠道 provider 测试计为 P0 验收。job scheduling 和资源库全局产物/adapter 继续由独立契约维护。
...
- `CON-22a`：既有 `veadk mpa create` 保持为向后兼容的基础设施部署命令。Studio 控制面等价能力放在独立的 `veadk mpa control` 命令组，包含 `view`、`create`、`update`、`operation list|get|retry`、`profile status|apply`、`session config-get|config-patch|profile-upgrade` 和 `delete-preview`。命令组从显式参数或受控环境变量接收 Studio BFF 地址和 bearer，绝不输出 bearer；Profile/request/change payload 从 JSON 文件或 stdin 读取。BFF 契约要求幂等键的写操作必须显式传入由调用方持久复用的 key；CAS 命令必须显式传入 ETag 或 Runtime revision。成功结果与控制面错误使用 JSON；本地选项/输入错误使用 Click 的 usage-error 文本。退出码固定为：`0` 成功、Click 的 `2` 表示本地用法/输入错误、`3` 表示 `401/403`、`4` 表示 `409/412/428`、`5` 表示可重试 transport/server 失败、`1` 表示其他已校验的控制面失败。对话、Turn 执行和 Runtime 诊断继续使用现有 Studio 执行面/诊断面；该 CLI 不直连 Runtime，也不为不存在的 BFF 契约宣称等价能力。
...
- `CON-30`：MPA 删除采用“先预览、再执行”的契约。`GET /web/mpa/agents/{mpaInstanceId}/delete-preview` 解析已授权 Runtime binding，读取 Runtime `profile-status`，调用现有 Runtime `GET /api/v1/sessions?include_a2a=true`，返回 `canDelete`、`blockers`、可见会话计数、活动会话摘要和清理计划。`/web/delete-runtime` 在 MPA 清理前重新检查服务端 Runtime 标签；浏览器传入的 `agentCategory` 只是提示，不能让普通 Runtime 进入 MPA 清理。`active` 状态 lifecycle operation 或状态为 `queued|running|pausing|paused|resuming` 的 active session 会在副作用前返回结构化 `409`。若某条遗留 lifecycle operation 为 `failed_retryable`，但 Runtime binding 已是 `bound` 且 Runtime `profile-status` 证明 Profile 已 `applied`，则不再把它作为删除 blocker；这样可以避免已恢复的 smoke 失败记录永久阻塞清理，同时保留真正 active operation 的保护。没有阻断时，Studio 先通过现有 Runtime `DELETE /api/v1/sessions/{sessionId}` 删除可见 idle/terminal Runtime 会话，再删除 AgentKit Runtime。单个 session `404` 表示已清理，会被忽略以支持安全重试。预览只统计已授权目标 Runtime API 可见的会话；若未来需要跨用户全局 session 状态，必须新增 Runtime/global admin endpoint 后再声明。
- `CON-26`: 仅后续：资源库、全局搜索、产物索引/来源 adapter。本期不开发、不验收。会话资源引用和会话结果仍在本期；MPA 模板/专家/审核不属于本方案。
- `CON-27`: 已有 MPA Runtime 的消息渠道绑定已在主干实现飞书、钉钉和企微，并由 `specs/mpa-channels` 管理，不再属于“代码未开发”的后续项。定时任务产品验收与网站 visitor/A2A 集成仍是独立后续。本 P0 不新增重复渠道 adapter、CLI 等价能力或 provider 专项 live 门禁。
...
2026-09-18：S5-09 设计评审新增 `CON-22a`，解决历史部署命令的名称冲突，并把 CLI parity 限定为 Studio BFF 已公开的管理面契约。该 CLI 切片不新增 Runtime 执行 endpoint、Managed Agent API 或持久数据模型。

2026-09-18：S5-09 已通过独立的 `veadk mpa control` 命令树实现 `CON-22a`。loopback 契约测试覆盖全部命令组、共享 schema 校验、Studio bearer 注入、202 operation、ETag/CAS、timeout 后稳定幂等键重放、旧 Runtime 只读输出、脱敏结构化错误和固定退出码。既有部署命令保持兼容，Runtime/API/数据模型不变。
...
S5-11 真实验证：`VEADK_MPA_P0_LIVE=1 VEADK_MPA_P0_ASYNC_STAGE_TIMEOUT_SECONDS=900 ./scripts/verify-mpa-p0-e2e.sh --manifest evidence/live/vc21-20260918-045/live-manifest.json --case VC-21 --execute` 返回 `{"case":"VC-21","residue":[],"status":"passed"}`。该 run 创建了隔离 Runtime、Tool 与 PostgreSQL database；应用 Studio Profile；通过真实 Worker 的 `ready -> create_session -> start_turn -> get_turn -> follow=false replay` 协议完成 execution-ready smoke；验证同镜像 no-op update、受控失败恢复、兼容 rollback smoke、active operation 与 active session 删除阻断、idle session 清理、最终 Runtime 删除和零残留。修复后的 CLI/route/driver 定向回归返回 49 passed，focused Ruff 通过。
