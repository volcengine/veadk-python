...
已实现的 S2-05 Profile 升级子集暴露 `POST /api/v1/sessions/{sessionId}/profile-upgrade`。该接口要求 `Idempotency-Key` 和当前 execution-config `If-Match`，只接受更高且已 applied 的 `targetProfileRevision`，保留显式 override，并基于目标 Profile 重新计算 inherit 默认值。相同 key/相同目标在当前进程内 replay 已创建 revision；相同 key/不同目标返回 `409 idempotency_mismatch`。跨进程持久 replay 仍需等待 SQL `runtime_operations` 实现完成。
