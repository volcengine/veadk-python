...
The implemented S2-05 profile-upgrade subset exposes `POST /api/v1/sessions/{sessionId}/profile-upgrade`. It requires `Idempotency-Key` and the current execution-config `If-Match`, accepts only a higher applied `targetProfileRevision`, preserves explicit overrides, and recomputes inherited defaults from the target Profile. Same-key/same-target retry replays the created revision within the current process; same-key/different-target returns `409 idempotency_mismatch`. Durable cross-process replay remains pending until the SQL `runtime_operations` implementation is completed.
