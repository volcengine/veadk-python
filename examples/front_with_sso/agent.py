...
"""A minimal agent for trying `veadk frontend` behind SSO login.

The SSO provider is configured entirely through environment variables on the
`veadk frontend` process — no VeIdentity user pool required. See README.md for
the GitHub / Google / custom-OIDC env vars and how to run it.
"""
