...
mock_user = User(id="veadk-user", email="user@example.com", group_memberships=["admin", "user"])
...
async def visualize_data(filename: str, title: str = None) -> str:
    ...
    if not filename.lower().endswith('.csv'):
        return f"Error: visualize_data only supports CSV files. You provided: {filename}"
...
async def read_file(filename: str, start_line: int = 1, end_line: int = -1) -> str:
    ...
    """
    args_model = read_file_tool.get_args_schema()(filename=filename, start_line=start_line, end_line=end_line)
...
async def save_correctanswer_memory(question: str, tool_name: str, args: Dict[str, Any]) -> str:
    """
...
async def generate_document(filename: str, content: str) -> str:
    ...
    """
    args_model = write_file_tool.get_args_schema()(filename=filename, content=content, overwrite=True)
