...
result_judge_agent: Agent = Agent(
    name="result_judge_agent",
    description="专门负责结果是否符合预期",
    instruction="""
### 任务
你是结果判断专家，专门负责：
- 对结果进行判断，判断是否符合预期
- 如果符合预期，调用工具 `exit_loop` 退出循环
- 如果不符合预期，无需调用工具 `exit_loop`，将会继续循环处理数据
    """,
    tools=[exit_loop],
    planner=PlanReActPlanner(),
    model_extra_config={"extra_body": {"thinking": {"type": "disabled"}}},
...
loop_agent: LoopAgent = LoopAgent(
    name="loop_agent",
    description="循环执行智能体",
    sub_agents=[
...
summary_agent: Agent = Agent(
    name="summary_agent",
    description="总结智能体，负责总结循环执行智能体的结果",
    instruction="""
### 任务
你是总结智能体，负责：
