...
metadata_query_agent: Agent = Agent(
    ...
    description="专门负责查询数据库元数据，理解用户查询中的数据对象、字段和过滤条件",
    instruction="""
### 任务
你是元数据查询专家，专门负责：
- 分析并理解用户查询中涉及的数据对象、字段、过滤条件等信息
- 根据用户自然语言请求，调用工具 `recall_metadata` 查询数据库元数据。注意，调用工具 `recall_metadata`的时候，tenant参数请固定为"c360"。
- 将用户自然语言映射到确切的数据库字段名
...
### DSL示例
{
    ...
    """,
    tools=[query_with_dsl, get_current_time],
    planner=PlanReActPlanner(),
    model_extra_config={"extra_body": {"thinking": {"type": "disabled"}}},
)
...
agent: Agent = Agent(
    ...
    description="主协调智能体，负责调度其他专业智能体完成复杂任务",
    instruction="""
### 任务
你是主协调智能体，负责：
...
### 调度策略
- 元数据查询：对于用户请求，首先调度 `metadata_query_agent` 进行查询涉及的数据库元数据（如表结构、字段描述等）
- 数据查询执行：基于用户请求和元数据，调度 `data_query_agent` 执行数据查询操作
- 复杂数据分析：如果用户请求是复杂的数据分析任务，必须调度 `data_analysis_agent` 进行分析
- 如果有需要，也可以非线性调度上述智能体，以应对用户复杂的查询需求。
...
如果查询不到有效信息，不要胡编数据，直接返回"查询不到有效信息"。
    ...
    tools=[
        ...
        visualize_data,
        summarize_data,
        AgentTool(metadata_query_agent),
        AgentTool(data_query_agent),
