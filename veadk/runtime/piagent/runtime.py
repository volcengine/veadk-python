                ...
                async with PiAgentRpcClient(run_config) as client:
                    ...
                    try:
                        ...
                        while True:
                            ...
                            # Parking them behind it would stall the live
                            # stream for the rest of the turn: a command's
                            # output and the final answer's deltas both arrive
                            # after the last durable event. Overtaking is safe
                            # because partials are never persisted
                            # (`BaseSessionService.append_event` returns early
                            # on them), so only the order among durable events
                            # is observable in session history, and that order
                            # is unchanged.
                            if event.partial:
                                yield event
                                continue
