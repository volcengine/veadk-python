...
def frontend(
    ...
    async def _deploy_to_agentkit(request: Request):
        ...
        try:
            ...
            # 3. Determine entry point and add server startup code
            entry_point = "app.py"
            if not (PathlibPath(temp_dir) / "app.py").exists():
                if (PathlibPath(temp_dir) / "agent.py").exists():
                    entry_point = "agent.py"
                else:
                    raise HTTPException(
                        status_code=400, detail="No app.py or agent.py found in files"
                    )

            # 4. Add server startup code to entry point if not present
            entry_path = PathlibPath(temp_dir) / entry_point
            entry_content = entry_path.read_text(encoding="utf-8")

            # Check if startup code already exists
            if "uvicorn.run" not in entry_content and "__name__" not in entry_content:
                # Add server startup code at the end
                startup_code = '''

# Auto-generated by AgentKit deployment
if __name__ == "__main__":
    print("=== VeADK: Starting FastAPI server ===")
    import uvicorn
    from google.adk.cli.fast_api import get_fast_api_app

    # Create FastAPI app from root_agent
    app = get_fast_api_app(root_agent=root_agent)
    uvicorn.run(app, host="0.0.0.0", port=8000)
'''
                entry_content += startup_code
                entry_path.write_text(entry_content, encoding="utf-8")
                logger.info(f"Added server startup code to {entry_point}")
