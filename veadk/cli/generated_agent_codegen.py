...
- 人物或虚构角色、品牌、产品、组织、平台或渠道、地点、日期、活动名称、具体题材、一次性问题或事件、文档标题、文件名、URL 等请求特有信息只能出现在 agents[*].task 中，不得出现在 name、id、description 或 instruction 中。即使品牌平台、产品品类或具体问题会影响执行方法，也只能通过“用户指定的平台/产品/问题”等参数化表达写入 instruction。禁止通过音译、拼音、翻译、缩写、拼接或轻微改写把这些特有信息写入名称。
...
- 在调用 create_agents 前逐字段自检每个 agents[*].name 和 nodes[*] 的 id、description、instruction。只要其中仍含本次任务特有的品牌、平台或渠道、产品或品类、故障或事件、人物、题材、文件名，或这些词的拼音、翻译、缩写，工具调用就是无效的，必须先改写为通用能力表达。使用“替换测试”检查：把当前请求的实体全部换成同领域的另一组实体后，这些身份字段应当保持不变。xiaohongshu、douyin、tiktok 等渠道都只能作为 task 中的运行时参数；智能手机、电池异常等产品类别和具体问题也只能作为 task 数据。
