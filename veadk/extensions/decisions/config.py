...
# Both deployments speak the same System One protocol; only the API base
# differs. "typesafe" is the hosted service, "systemone" is a self-hosted
# System One server.
DecisionProvider = Literal["typesafe", "systemone"]
...
class DecisionModelConfig(BaseModel):
    ...
    def from_env(cls, env: Mapping[str, str] | None = None) -> DecisionModelConfig:
        ...
        return cls(
            ...
            api_base=_lookup(values, "API_BASE") or DEFAULT_API_BASE,
...
def _env_provider(raw: str | None) -> DecisionProvider:
    ...
    if value in ("typesafe", "systemone"):
