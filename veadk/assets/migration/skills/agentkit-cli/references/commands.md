...
├─ config               create/update root agentkit.yaml and global defaults
├─ build                build from root agentkit.yaml
├─ deploy               deploy from root agentkit.yaml
├─ launch               build + deploy from root agentkit.yaml
├─ status               show status from root agentkit.yaml
├─ release              build & release the project (config → build → apply)
...
Scaffold a new project from a template key. Verify current support with
`agentkit init --list-templates`; keys are accepted by ID, key, or template name.
Common keys include `basic`, `basic_stream`, `a2a`, `agent_server`,
`langchain_basic_stream`, `basic_go`, `a2a_go`, `eino_a2a`, and `harness`.
Current `init` writes a root `agentkit.yaml` lifecycle config plus project files
such as `<project>.py`, `requirements.txt`, and `.dockerignore`.

```bash
agentkit init --list-templates
agentkit init my-agent --template basic --directory ./my-agent
agentkit init my-agent --template basic_stream --directory ./my-agent
agentkit init wrapped-agent --from-agent ./agent.py --agent-var agent --wrapper-type basic --directory ./wrapped-agent
```

## config / build / deploy / launch / status

These lifecycle commands operate on root `agentkit.yaml` and user-wide defaults.
They are useful for inspecting or updating the current init output, provider,
region, and runtime bindings. This lifecycle flow is separate from the
`.agentkit/agentkit.yaml` release flow below.

```bash
agentkit config --show
agentkit config --global --show
agentkit config --cloud_provider byteplus --region ap-southeast-1
agentkit build
agentkit deploy
agentkit launch
agentkit status --verbose
```

## release

One-shot build & release of the current project (runs config → build → apply).
If `.agentkit/agentkit.yaml` is absent, the first bare `agentkit release --name`
scaffolds it and exits for review; run `agentkit release` again after confirming
the generated config.

```bash
agentkit release --name my-agent --project default
agentkit release
```

### release config

Scaffold `.agentkit/` (`agentkit.yaml` + `Dockerfile`) for the release flow.
This is separate from root `agentkit.yaml`, which is produced by `init` and used
by lifecycle commands such as `config`, `build`, `deploy`, `launch`, and `status`.

```bash
agentkit release config --name my-agent
```

`agentkit release` is fully **yaml-driven** — everything lives in
