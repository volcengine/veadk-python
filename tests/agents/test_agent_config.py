...
from veadk.agents.loop_agent import LoopAgent as GoogleADKLoopAgent
from veadk.agents.parallel_agent import ParallelAgent as GoogleADKParallelAgent
from veadk.agents.sequential_agent import SequentialAgent as GoogleADKSequentialAgent
...
def test_agent_config_discriminator_llm_agent(agent_class_value: str, tmp_path: Path):
    yaml_content = f"""\
...
@pytest.mark.parametrize(
    ...
    [
        "LoopAgent",
        "veadk.agents.loop_agent.LoopAgent",
        "google.adk.agents.loop_agent.LoopAgent",
    ],
)
def test_agent_config_discriminator_loop_agent(agent_class_value: str, tmp_path: Path):
    yaml_content = f"""\
agent_class: {agent_class_value}
...
"""
    ...
    # Check if the agent is an instance of either LoopAgent class
    assert (
        isinstance(agent, LoopAgent)
        or isinstance(agent, GoogleADKLoopAgent)
        or type(agent).__name__ == "LoopAgent"
    )
    assert config.root.agent_class == agent_class_value
...
@pytest.mark.parametrize(
    ...
    [
        "ParallelAgent",
        "veadk.agents.parallel_agent.ParallelAgent",
...
"""
    ...
    # Check if the agent is an instance of either ParallelAgent class
    assert (
        isinstance(agent, ParallelAgent)
        or isinstance(agent, GoogleADKParallelAgent)
        or type(agent).__name__ == "ParallelAgent"
    )
    assert config.root.agent_class == agent_class_value
...
@pytest.mark.parametrize(
    ...
    [
        "SequentialAgent",
        "veadk.agents.sequential_agent.SequentialAgent",
...
"""
    ...
    # Check if the agent is an instance of either SequentialAgent class
    assert (
        isinstance(agent, SequentialAgent)
        or isinstance(agent, GoogleADKSequentialAgent)
        or type(agent).__name__ == "SequentialAgent"
    )
    assert config.root.agent_class == agent_class_value
...
@pytest.mark.parametrize(
    ...
    [
        ("LoopAgent", LoopAgent),
        ("veadk.agents.loop_agent.LoopAgent", LoopAgent),
        ("google.adk.agents.loop_agent.LoopAgent", LoopAgent),
        ("ParallelAgent", ParallelAgent),
        ("veadk.agents.parallel_agent.ParallelAgent", ParallelAgent),
        ("google.adk.agents.parallel_agent.ParallelAgent", ParallelAgent),
        ("SequentialAgent", SequentialAgent),
        ("veadk.agents.sequential_agent.SequentialAgent", SequentialAgent),
...
"""
    (sub_agent_dir / "sub_agent1.yaml").write_text(sub_agent_config.format(index=1))
    (sub_agent_dir / "sub_agent2.yaml").write_text(sub_agent_config.format(index=2))
    yaml_content = f"""\
agent_class: {agent_class_value}
name: main_agent
...
"""
    ...
    # Check if the agent is an instance of the expected agent type or its Google ADK equivalent
    if expected_agent_type is LoopAgent:
        assert (
            isinstance(agent, LoopAgent)
            or isinstance(agent, GoogleADKLoopAgent)
            or type(agent).__name__ == "LoopAgent"
        )
    elif expected_agent_type is ParallelAgent:
        assert (
            isinstance(agent, ParallelAgent)
            or isinstance(agent, GoogleADKParallelAgent)
            or type(agent).__name__ == "ParallelAgent"
        )
    elif expected_agent_type is SequentialAgent:
        assert (
            isinstance(agent, SequentialAgent)
            or isinstance(agent, GoogleADKSequentialAgent)
            or type(agent).__name__ == "SequentialAgent"
...
"""
    ...
    my_custom_config = MyCustomAgentConfig.model_validate(config.root.model_dump())
